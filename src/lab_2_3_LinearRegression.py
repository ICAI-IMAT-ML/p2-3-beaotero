...
class LinearRegressor:
    ...
    def fit_simple(self, X, y):
        ...
        covarianza_x_y = 0
        varianza_x = 0
        covarianza_x_y =np.sum((X- np.mean(X))*(y-np.mean(y)))
        varianza_x = np.sum((X - np.mean(X))**2)
        w = covarianza_x_y/varianza_x
    ...
    def fit_multiple(self, X, y):
        ...
        X_bias = np.c_[np.ones(X.shape[0]), X]  # Añadimos un acolumna de unos para el intercepto
        
        self.coefficients = np.linalg.inv(np.transpose(X_bias) @ X_bias) @ np.transpose(X_bias) @ y

        self.intercept = self.coefficients[0]  # Primer elemento es el intercepto
        self.coefficients = self.coefficients[1:]  # Los demás son los coeficientes
